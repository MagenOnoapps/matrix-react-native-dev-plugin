...
description: >-
  ...
  detailed Draft audit document under an audits topic folder, then updates
  AUDIT.md with the created file path and Draft status. It never modifies
  source code and never proceeds to another topic without explicit developer
  approval. Part of the project-inspector workflow.
---
...
This skill is step 3 of the project-inspector workflow:
...
4. The developer reviews and approves the Draft; `audit-sync` then syncs approved findings into `CLAUDE.md`.
...
While the marker exists, the plugin's PreToolUse hook mechanically blocks destructive shell commands. As defense-in-depth, still follow these rules yourself:
...
- Write files only through the Write/Edit tools, and only the two files in the Output Contract.
...
7. Activate the read-only guard marker.
8. Inspect only the repository areas relevant to the selected topic.
9. Create `audits/<topic-slug>/` if needed.
10. Generate `audits/<topic-slug>/<topic-slug>-audit.md` from `templates/topic-audit.template.md`.
11. Update the selected row in `AUDIT.md`:
    - `Status` -> `Draft`
    - `File` -> relative path of the created audit file
12. Deactivate the read-only guard marker.
13. Stop and report completion.
14. Do not continue to the next topic until the developer explicitly asks to continue.
...
Wait for the developer's answers before proceeding. If the developer already provided a topic as a command argument (e.g., `/inspect-breakdown Networking`), use it as the selected topic and skip question 3.
...
Do not repeat a full `project-analysis` scan. If `docs/project/` exists (generated by `project-docs`), read the relevant knowledge-base file first — it often already maps the area under audit.

**For broad topic scans, delegate to the `repo-scanner` agent** with a topic-focused brief (e.g., "find all Manager/Service/Singleton classes in <repo> and summarize their responsibilities and coupling"). Use its structured digest as the evidence base.
...
## Step 7: Generate the Domain Audit File

Read `templates/topic-audit.template.md` (relative to this skill) and use it exactly as the structure for the generated topic audit file.
...
Use the `[HIGH / MEDIUM / LOW]` severity tags consistently — the `audit-sync` skill parses them when syncing approved findings into `CLAUDE.md`.
...
After processing the one selected topic and removing the guard marker, report:
...
Next:
  ...
  If approved, run /inspect-sync to mark it Approved and sync findings into CLAUDE.md,
  or ask me to continue to the next Pending Breakdown topic.
...
- Never create `docs/` content — that is owned by the `project-docs` skill.
...
- Never mark a Draft as Approved. Only the developer can approve (via the `audit-sync` skill).
