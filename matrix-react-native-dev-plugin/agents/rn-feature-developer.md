...
3. Implement against every applicable standard for the surface being touched:
...
4. Self-check the change against the task's acceptance criteria before reporting done.
5. Report which standard IDs were actually applied (not just "reviewed") — this is what `rn-code-reviewer`, `rn-performance-reviewer`, and QA handoff trace back to.
